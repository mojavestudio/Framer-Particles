...
// Complete configuration interface matching the original
...
// Generate the complete particles component code with ALL original features
function createParticlesCode(config: ParticleConfig): string {
    return `// @framerDisableUnlink
/**
...
export default function MojaveParticles(props: any) {
    // Configuration with defaults from plugin
    const backdrop = props?.backdrop || "${config.backdrop}"
    const backgroundOpacity = props?.backgroundOpacity !== undefined ? props.backgroundOpacity : ${config.backgroundOpacity}
    const color = props?.color || "${config.color}"
    const colors = props?.colors || ${JSON.stringify(config.colors)}
    const amount = props?.amount || ${config.amount}
    const size = props?.size || ${JSON.stringify(config.size)}
    const opacity = props?.opacity || ${JSON.stringify(config.opacity)}
    const radius = props?.radius !== undefined ? props.radius : ${config.radius}
    const width = props?.width || ${config.width}
    const height = props?.height || ${config.height}
    const twinkle = props?.twinkle || ${JSON.stringify(config.twinkle)}
    const modes = props?.modes || ${JSON.stringify(config.modes)}
    const move = props?.move || ${JSON.stringify(config.move)}
    const click = props?.click || ${JSON.stringify(config.click)}
    const hover = props?.hover || ${JSON.stringify(config.hover)}
    ...
    useEffect(() => {
        ...
        // Set additional context properties for sharp rendering
    ...
    }, [width, height, amount, size, opacity, color])
...
export function App() {
    ...
    // Complete configuration state matching original
    ...
    const handleAddParticles = async () => {
        setIsLoading(true)
        setMessage("Creating your fully-featured particles...")
        ...
        try {
            ...
            if (defaultExport && 'insertURL' in defaultExport && defaultExport.insertURL) {
                ...
                try {
                    ...
                    setMessage("🎉 Complete Mojave Particles system added! All features protected.")
                    
                    // Auto-refresh existing particles list
                    setTimeout(() => findExistingParticles(), 1000)
                } catch (error) {
                    console.error("Error adding component:", error)
                    setMessage("✅ Advanced particles created! Drag from Assets to add to canvas.")
                }
            } else {
                setMessage("✅ Full-featured particles component created successfully!")
            }
        } catch (error) {
            const errorMessage = error instanceof Error ? error.message : String(error)
            if (errorMessage.includes("permission")) {
                setMessage("❌ Permission required. Please enable required permissions in Framer plugin settings.")
            } else {
    ...
    return (
        <main style={{ 
            ...
            <div style={{ 
                                ...
                                {existingParticles.map((particle) => (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            alignItems: 'center',
                                            padding: '10px',
                                            border: selectedParticle === particle.id ? '2px solid #007aff' : '1px solid #eee',
                                            borderRadius: '6px',
                                            cursor: 'pointer',
                                            background: selectedParticle === particle.id ? '#f0f8ff' : 'white'
                                        }}
                                        onClick={() => {
                ...
                {/* Awesome Presets */}
                <div style={{ marginTop: '10px', marginBottom: '20px', background: 'var(--card-bg)', padding: '15px', borderRadius: '8px', border: '1px solid var(--border)' }}>
                    <label style={{ fontSize: '12px', fontWeight: 'bold', color: 'var(--text)', marginBottom: '12px', display: 'block' }}>
                        <Palette size={14} style={{ marginRight: '6px', verticalAlign: 'middle', display: 'inline-block' }} />
                        Awesome Presets
                    </label>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                        ...
                        <button
                            ...
                            style={{
                                padding: '10px 12px',
                                border: '1px solid var(--border)',
                                background: 'linear-gradient(135deg, #0a0a2a 0%, #2d1b69 30%, #5b21b6 60%, #7c3aed 100%)',
                                borderRadius: '6px',
                                fontSize: '11px',
                                cursor: 'pointer',
                                color: '#e5e7eb',
